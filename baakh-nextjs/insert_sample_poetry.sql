...
-- This script adds sample poetry entries to test the /sd/categories/[slug] endpoints
...
DECLARE
    nazm_cat_id INTEGER;
    ghazal_cat_id INTEGER;
    tairru_cat_id INTEGER;
    chhasitta_cat_id INTEGER;
    sample_poet_id UUID;
BEGIN
    -- Get category IDs
    ...
    SELECT id INTO sample_poet_id FROM public.poets LIMIT 1;
    IF sample_poet_id IS NULL THEN
        ...
        RETURNING id INTO sample_poet_id;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM public.poetry_main WHERE poetry_slug = 'sample-nazm-1') THEN
        INSERT INTO public.poetry_main (
            ...
            ARRAY['نظم', 'صبح', 'طبيعت'], 'sd', 'public', true
        );
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM public.poetry_main WHERE poetry_slug = 'sample-nazm-2') THEN
        INSERT INTO public.poetry_main (
            ...
            ARRAY['نظم', 'رات', 'خواب'], 'sd', 'public', false
        );
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM public.poetry_main WHERE poetry_slug = 'sample-ghazal-1') THEN
        INSERT INTO public.poetry_main (
            ...
            ARRAY['غزل', 'محبت', 'دل'], 'sd', 'public', true
        );
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM public.poetry_main WHERE poetry_slug = 'sample-tairru-1') THEN
        INSERT INTO public.poetry_main (
            ...
            ARRAY['تيرڙو', 'طبيعت', 'هوائي'], 'sd', 'public', false
        );
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM public.poetry_main WHERE poetry_slug = 'sample-chhasitta-1') THEN
        INSERT INTO public.poetry_main (
            ...
            ARRAY['چهاستا', 'ڳوٺ', 'لوڪ'], 'sd', 'public', false
        );
    END IF;
END $$;
