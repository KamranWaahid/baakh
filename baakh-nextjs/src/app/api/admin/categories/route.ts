...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    if (body?.restore === true) {
      const { error: restoreErr } = await supabase.from("categories").update({ deleted_at: null }).eq("id", id);
    ...
    const { error: upErr } = await supabase.from("categories").update(updateCat).eq("id", id);
    ...
    const { error: detErr } = await supabase.from("category_details").upsert(detailsPayload, { onConflict: "cat_id,lang" });
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    const { error: softErr } = await supabase.from("categories").update({ deleted_at: new Date().toISOString() }).eq("id", id);
    ...
    if (!catId) {
      ...
      if (catErr) {
        ...
        if (code === "23505" || msg.includes("duplicate key value") || msg.includes("categories_slug_key")) {
          const { data: foundAgain } = await supabase
            .from("categories")
            .select("id")
            .eq("slug", slug)
            .limit(1);
